...
import gpayments
from gpayments import error, util
...
class Webhook(object):
    ...
    def construct_event(payload, sig_header, secret,
                        tolerance=DEFAULT_TOLERANCE, access_token=None):
        if hasattr(payload, 'decode'):
            payload = payload.decode('utf-8')
        if access_token is None:
            access_token = gpayments.access_token
        data = util.json.loads(payload)
        event = gpayments.Event.construct_from(data, access_token)
